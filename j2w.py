...
st.set_page_config(page_title="Convert PDF to Word with Pandoc & Images", page_icon="📐", layout="wide")

st.title("📐 Convert PDF to Word (Mistral OCR + Pandoc + Image Embedding)")
...
if uploaded_pdf and st.button("🚀 Gửi PDF lên Mistral OCR"):
    if not mistral_api_key:
        ...
        with st.spinner("Đang gửi PDF lên Mistral OCR API, xử lý ảnh và tạo file Word..."):
            try:
                            ...
                            for img in page.images:
                                if hasattr(img, "id") and hasattr(img, "image_base64") and img.image_base64:
                                    ...
                                    try:
                                        ...
                                        # QUAN TRỌNG: Lưu file ảnh vật lý ra thư mục gốc để Pandoc tìm thấy và nhúng vào docx
                                        img_path = os.path.join(root_dir, img_filename)
                                        with open(img_path, "wb") as img_f:
                                            img_f.write(img_bytes)
                                    except: pass

                # 3. Gọi Pandoc biên dịch Markdown sang Word (Pandoc tự động đọc các file ảnh cùng tên trong thư mục gốc)
                ...
                # Dọn dẹp file markdown tạm
                if os.path.exists(temp_md_path):
                    os.remove(temp_md_path)
                    
                st.success("🎉 Xử lý thành công! Ảnh đã được nhúng vào file Word và hiển thị ở khung xem trước.")
...
if st.session_state.preview_markdown:
    ...
    # Xử lý hiển thị base64 ảnh cho khung preview trực quan trên web
